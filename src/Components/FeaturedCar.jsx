...
const FeaturedCar = () => {
    ...
    useEffect(() => {
        const filtered = cars.filter(car => car.carName.toLowerCase().includes(searchQuery.toLowerCase()))
    ...
    return (
        <section className="py-10 px-4 max-w-7xl mx-auto">
            ...
            <input
                ...
                className="border p-2 rounded w-full md:w-1/4 mb-4 ml-2"
            />
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredCars.length > 0 ? (cardCars) : (<p className='mt-6 text-center text-gray-500'>No cars found.</p>)}
            </div>
        </section>
    )
};
